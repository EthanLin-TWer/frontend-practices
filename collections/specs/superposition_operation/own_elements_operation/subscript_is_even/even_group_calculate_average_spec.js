...
describe('even_group_calculate_average', function() {
  ...
  it.skip('首先选出所有第偶数个元素，然后选出其中的偶数，按几位数分组，并计算每组的平均数', function() {
    const result = even_group_calculate_average(collection_a)
    expect(result).toEqual([4, 56, 556])
  })

  it.skip('首先选出所有第偶数个元素，当不含有偶数时', function() {
    const result = even_group_calculate_average(collection_b)
    expect(result).toEqual([0])
  })

  it.skip('首先选出所有第偶数个元素，然后选出其中的偶数，按几位数分组，当不含有1位，2位的数字，但含有3位的情况时，计算这组的平均数', function() {
    const result = even_group_calculate_average(collection_c)
